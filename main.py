...
class substance:
  ...
  def __repr__(self):
    ret = "<%s %d> COST = %d CO = %d SUBS:" % (self.name, self.count, self.cost, self.co)
    for i in self.subs:
      ret += i
      ret += " "
...
def reccount(sub,count):
  global substances
  #print(sub)
  ret = 0
  su = substances[sub]
  co = math.floor(count / su.count)
  n1 = int(count) % int(su.count)
  if n1 != 0:
    if n1 > su.left:
      su.left += su.count - n1
      co += 1
      ...
      su.left -= n1
  for s in range(len(su.subs)):
    if su.subs[s] == "ORE":
      ret += math.floor(su.counts[s] * co)
    else:
      #print(su.counts[s] * co)
      ret += reccount(su.subs[s],math.floor(su.counts[s] * co))
  #print(sub)
  #print(ret)
  su.co += co
  su.cost += ret
...
for line in inp:
  raw = line.split(" ")
  substances[raw[-1]] = substance(raw[-1],int(raw[-2]))
  itr =len(raw) - 4
  while itr != -1:
    substances[raw[-1]].subs.append(raw[itr].strip(','))
    substances[raw[-1]].counts.append(int(raw[itr-1]))
    itr -= 2
